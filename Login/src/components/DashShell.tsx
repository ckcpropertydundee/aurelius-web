...
export default function DashShell({ tabs, active, onChange, metrics, children, userInitials }: Props) {
  ...
  const { user } = useAuth()
  const showListings = ['admin', 'master admin', 'staff'].includes(user?.role ?? '')
  ...
  return (
    <div className="min-h-dvh flex" style={{ background: '#0d1b2e' }}>
      ...
      >
        ...
        {/* View listings link — admin and staff only */}
        {showListings && <div className="px-4 pb-5" style={{ borderTop: '1px solid rgba(255,255,255,0.07)', paddingTop: 12 }}>
          <a
            href="https://login.aureliuspropertymanagement.co.uk/for-let"
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-3 px-3 py-2.5 rounded w-full transition-colors"
            style={{ color: '#8899aa', textDecoration: 'none' }}
            onMouseEnter={e => (e.currentTarget.style.color = '#e8edf5')}
            onMouseLeave={e => (e.currentTarget.style.color = '#8899aa')}
          >
            <span className="rounded-full flex-shrink-0" style={{ width: 4, height: 4, background: 'currentColor' }} />
            <span style={{ fontSize: 11, letterSpacing: '0.12em', textTransform: 'uppercase' }}>View Listings</span>
            <span style={{ fontSize: 10, marginLeft: 'auto', opacity: 0.5 }}>↗</span>
          </a>
        </div>}
      </aside>
